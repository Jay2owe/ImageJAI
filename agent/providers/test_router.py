...
def test_provider_key_allow_list_and_proxy_count() -> None:
    ...
    assert len(_PROXY_PROVIDERS) == 17


def test_local_daemon_providers_route_to_proxy_client() -> None:
    # The keyless local OpenAI-compatible servers go through the LiteLLM proxy
    # client (only the api_base differs) — no native client, no required key.
    for provider in ("lmstudio", "jan", "llamacpp", "vllm"):
        client = router.get_client(provider, "some-loaded-model",
                                   base_url="http://localhost:4000")
        assert isinstance(client, LiteLLMProxyClient), provider


def test_local_daemon_providers_are_wildcard_and_skip_preflight() -> None:
    # Discovered (loaded) local models are not curated aliases, so the proxy
    # routes them via a "<prefix>/*" wildcard and the launch preflight must NOT
    # require the exact alias to be enumerated. wildcard short-circuits with no
    # network call to /v1/models.
    for provider in ("lmstudio", "jan", "llamacpp", "vllm"):
        client = router.get_client(provider, "any-loaded-model",
                                   base_url="http://localhost:4000")
        assert client.wildcard is True, provider
        assert client.require_model_available("any-loaded-model") == (
            f"{provider}/any-loaded-model"
        )
    # Keyed cloud providers are NOT wildcard (strict preflight preserved).
    groq = router.get_client("groq", "llama-3.3-70b-versatile",
                             base_url="http://localhost:4000")
    assert getattr(groq, "wildcard", False) is False
...
def test_ollama_cloud_proxy_smoke_routes_to_litellm_alias() -> None:
    _assert_proxy_smoke(
        "ollama-cloud",
        "gemma4:31b-cloud",
