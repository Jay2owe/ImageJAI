...
from agent.providers import litellm_proxy, router
from agent.providers.litellm_proxy import COST_HEADER, LiteLLMProxyClient
...
def test_litellm_proxy_emits_cost_header_to_budget_listeners() -> None:
    client = router.get_client("groq", base_url="http://localhost:4000", api_key="dummy")
    seen: list[str] = []
    listener = seen.append
    litellm_proxy.add_cost_listener(listener)
    try:
        with respx.mock(assert_all_called=True) as mock:
            mock.post("http://localhost:4000/v1/chat/completions").mock(
                return_value=httpx.Response(
                    200,
                    headers={COST_HEADER: "0.017"},
                    json=_openai_text_json("ok"),
                )
            )
            client.chat([{"role": "user", "content": "hi"}], [],
                        "llama-3.3-70b-versatile")
    finally:
        litellm_proxy.remove_cost_listener(listener)
...
def test_require_model_available_accepts_proxy_alias() -> None:
    ...
    with respx.mock(assert_all_called=True) as mock:
        mock.get("http://localhost:4000/v1/models").mock(
            return_value=httpx.Response(
                200,
                json={"data": [{"id": "ollama-cloud/gemma4:31b-cloud"}]},
            )
        )
        assert client.require_model_available("gemma4:31b-cloud") == (
            "ollama-cloud/gemma4:31b-cloud"
        )


def test_require_model_available_reports_missing_alias() -> None:
    client = router.get_client(
        "ollama-cloud",
        base_url="http://localhost:4000",
        api_key="dummy",
    ...
    with respx.mock(assert_all_called=True) as mock:
        mock.get("http://localhost:4000/v1/models").mock(
            return_value=httpx.Response(
                200,
                json={"data": [{"id": "ollama-cloud/qwen3-coder:480b-cloud"}]},
            )
        )
        with pytest.raises(RuntimeError, match="does not expose model alias"):
            client.require_model_available("gemma4:31b-cloud")
