...
def load_context(model_id: str) -> str:
    ...
    if family:
        # Strict, exactly like every overlay above: a models.yaml entry whose
        # `family` names a missing families/<family>.md is a curation bug and
        # must fail loudly (FileNotFoundError) rather than silently composing a
        # generic context with the family rules dropped.
