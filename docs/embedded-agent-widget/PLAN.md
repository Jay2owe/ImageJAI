...
(`src/main/java/uk/ac/ucl/imagej/ai/ui/ChatPanel.java:178-193`). Stuffing a
...
plugin (Fiji ships many LGPL plugins). Confirm with UCL licensing before
...
(`src/main/java/uk/ac/ucl/imagej/ai/engine/AgentLauncher.java:112-167`), and
`ChatPanel.showAgentLaunchMenu` only reads success/fail
(`src/main/java/uk/ac/ucl/imagej/ai/ui/ChatPanel.java:946-949`). That interface
...
- New `uk.ac.ucl.imagej.ai.engine.AgentLaunchSpec` extracted from
  `AgentLauncher` so command construction, cwd, and environment are
  shared by external and embedded launch modes.
- New interface `uk.ac.ucl.imagej.ai.engine.AgentSession`.
- `ExternalAgentSession` — refactored from today's
  `AgentLauncher.launchAgent()` body
  (`src/main/java/uk/ac/ucl/imagej/ai/engine/AgentLauncher.java:112-167`),
...
- `ChatPanel.showAgentLaunchMenu`
  (`src/main/java/uk/ac/ucl/imagej/ai/ui/ChatPanel.java:905-952`) updated
  to accept `AgentSession`; continues to discard the handle for now.
- New `uk.ac.ucl.imagej.ai.ui.ChatSurface` (or equivalent tiny facade)
...
- `Settings.agentEmbeddedTerminal` flag (in
  `src/main/java/uk/ac/ucl/imagej/ai/config/Settings.java`), surfaced in
  `src/main/java/uk/ac/ucl/imagej/ai/ui/SettingsDialog.java`.
...
- **Session code journal — server side only** (§2.10). New class
  `uk.ac.ucl.imagej.ai.engine.SessionCodeJournal` with the
...
- `uk.ac.ucl.imagej.ai.terminal.EmbeddedPty` wrapping
...
- `ImageJAIPlugin.run()`
  (`src/main/java/uk/ac/ucl/imagej/ai/ImageJAIPlugin.java:40-135`) swaps
...
Deliverables:
- `uk.ac.ucl.imagej.ai.ui.LeftRail` (`JPanel`, `BoxLayout.Y_AXIS`),
  collapsible via a chevron.
- `uk.ac.ucl.imagej.ai.ui.TcpHotline` helper: opens a short-lived
...
Deliverables:
- **Prompt-watcher** (`uk.ac.ucl.imagej.ai.terminal.PromptWatcher`):
...
- **Terminal toolbar** (`uk.ac.ucl.imagej.ai.ui.TerminalToolbar`,
...
- **Session history section** on the `LeftRail` (§2.10). New
  `uk.ac.ucl.imagej.ai.ui.SessionHistoryPanel` subscribed to
...
| `src/main/java/uk/ac/ucl/imagej/ai/engine/AgentLaunchSpec.java` | NEW — shared shell command / cwd / env contract |
| `src/main/java/uk/ac/ucl/imagej/ai/engine/AgentSession.java` | NEW — interface |
| `src/main/java/uk/ac/ucl/imagej/ai/engine/ExternalAgentSession.java` | NEW — wraps today's launcher behaviour as a detached-session stub |
| `src/main/java/uk/ac/ucl/imagej/ai/engine/EmbeddedAgentSession.java` | NEW — wraps pty4j |
| `src/main/java/uk/ac/ucl/imagej/ai/terminal/EmbeddedPty.java` | NEW — pty4j + JediTerm + connector |
| `src/main/java/uk/ac/ucl/imagej/ai/terminal/ImageJAITtyConnector.java` | NEW |
| `src/main/java/uk/ac/ucl/imagej/ai/terminal/PromptWatcher.java` | NEW — 250 ms poll over scrollback, matches approval.json regexes, emits events |
| `src/main/java/uk/ac/ucl/imagej/ai/terminal/ApprovalPolicy.java` | NEW — loads approval.json, decides auto_confirm / escalate / pending |
| `src/main/java/uk/ac/ucl/imagej/ai/ui/TerminalToolbar.java` | NEW — Confirm / Cancel / Interrupt / Kill / Copy URL buttons above JediTerm pane |
| `src/main/java/uk/ac/ucl/imagej/ai/ui/AiRootPanel.java` | NEW — header + CardLayout body |
| `src/main/java/uk/ac/ucl/imagej/ai/ui/ChatSurface.java` | NEW — tiny UI facade for ConversationLoop / TCP wiring |
| `src/main/java/uk/ac/ucl/imagej/ai/ui/ChatView.java` | NEW — extracted body of today's ChatPanel |
| `src/main/java/uk/ac/ucl/imagej/ai/ui/TerminalView.java` | NEW — rail + terminal host |
| `src/main/java/uk/ac/ucl/imagej/ai/ui/TerminalHost.java` | NEW — holds JediTermWidget |
| `src/main/java/uk/ac/ucl/imagej/ai/ui/LeftRail.java` | NEW |
| `src/main/java/uk/ac/ucl/imagej/ai/ui/TcpHotline.java` | NEW |
| `src/main/java/uk/ac/ucl/imagej/ai/engine/SessionCodeJournal.java` | NEW — §2.10 silent capture, auto-namer, ring + on-disk persistence |
| `src/main/java/uk/ac/ucl/imagej/ai/engine/CodeAutoNamer.java` | NEW — §2.10 cascading name strategies (comment → composite run-signature → first symbol → timestamp) |
| `src/main/java/uk/ac/ucl/imagej/ai/ui/SessionHistoryPanel.java` | NEW — rail section, single/double-click, right-click menu |
| `AI_Exports/.session/code/` | NEW — runtime-created, per-entry `.ijm`/`.groovy` files + `INDEX.json` |
| `src/main/java/uk/ac/ucl/imagej/ai/engine/AgentLauncher.java` | MODIFY — return `AgentSession`, accept `Mode`, extract `AgentLaunchSpec`, fix shell-portable command building |
| `src/main/java/uk/ac/ucl/imagej/ai/ui/ChatPanel.java` | MODIFY — Phase 0 launcher/session plumbing, then compatibility wrapper while `ChatView` extraction lands |
| `src/main/java/uk/ac/ucl/imagej/ai/ImageJAIPlugin.java` | MODIFY — swap to `AiRootPanel`, shutdown hook |
| `src/main/java/uk/ac/ucl/imagej/ai/ConversationLoop.java` | MODIFY — `java.util.Base64`, depend on `ChatSurface` instead of concrete `ChatPanel` |
| `src/main/java/uk/ac/ucl/imagej/ai/config/Settings.java` | MODIFY — `agentEmbeddedTerminal` flag |
| `src/main/java/uk/ac/ucl/imagej/ai/ui/SettingsDialog.java` | MODIFY — surface the flag |
...
1. **`AgentLauncher` refactor blast radius** (was underweighted in draft 1).
   ...
   (`src/main/java/uk/ac/ucl/imagej/ai/ui/ChatPanel.java:946-949`).
...
- **"Explain this dialog" button.** `agent/probe_plugin.py:49`
  ...
  (`src/main/java/uk/ac/ucl/imagej/ai/engine/TCPCommandServer.java:3566`)
