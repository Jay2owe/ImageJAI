...
1. **Chat/pipeline macro timeouts can leave zombie macros running, and those paths still allow overlapping `IJ.runMacro()` calls.**
   Refs: `src/main/java/imagejai/engine/CommandEngine.java:76-87`, `src/main/java/imagejai/engine/CommandEngine.java:196-205`, `src/main/java/imagejai/engine/CommandEngine.java:225-299`, `src/main/java/imagejai/engine/TCPCommandServer.java:94-105`, `src/main/java/imagejai/engine/TCPCommandServer.java:1199-1319`
...
2. **The non-TCP macro runner still reports some real ImageJ failures as success.**
   Refs: `src/main/java/imagejai/engine/CommandEngine.java:80-130`, `src/main/java/imagejai/engine/CommandEngine.java:266-299`, `src/main/java/imagejai/engine/TCPCommandServer.java:1167-1180`, `src/main/java/imagejai/engine/TCPCommandServer.java:1239-1244`, `src/main/java/imagejai/engine/TCPCommandServer.java:1311-1317`, `src/main/java/imagejai/engine/TCPCommandServer.java:1491-1617`
...
3. **Runtime settings changes are saved, but not applied to the live backend/server/launcher.**
   Refs: `src/main/java/imagejai/ui/ChatPanel.java:721-724`, `src/main/java/imagejai/ui/ChatPanel.java:861-864`, `src/main/java/imagejai/ui/ChatPanel.java:885-892`, `src/main/java/imagejai/ConversationLoop.java:74-85`, `src/main/java/imagejai/ImageJAIPlugin.java:71-83`, `src/main/java/imagejai/ImageJAIPlugin.java:85-87`
...
4. **Subscribe detection can hijack non-subscribe requests before real parsing.**
   Refs: `src/main/java/imagejai/engine/TCPCommandServer.java:350-365`, `src/main/java/imagejai/engine/TCPCommandServer.java:395-398`
...
5. **Agent workspace discovery is developer-specific, and the fallback workspace is effectively empty.**
   Refs: `src/main/java/imagejai/ImageJAIPlugin.java:159-181`
...
6. **The predefined Open Interpreter launch command is POSIX-only and broken on Windows.**
   Refs: `src/main/java/imagejai/engine/AgentLauncher.java:51`, `src/main/java/imagejai/engine/AgentLauncher.java:126-135`
...
7. **Event coalescing drops different events on the same topic, not just duplicates.**
   Refs: `src/main/java/imagejai/engine/EventBus.java:83-96`
...
8. **`job.completed` frames can publish a negative `elapsedMs`.**
   Refs: `src/main/java/imagejai/engine/JobRegistry.java:234-255`, `src/main/java/imagejai/engine/JobRegistry.java:296-300`
...
9. **The TCP server creates one unbounded thread per client connection.**
   Refs: `src/main/java/imagejai/engine/TCPCommandServer.java:282-297`
...
10. **Ollama model refresh uses brittle string scanning instead of JSON parsing.**
    Refs: `src/main/java/imagejai/ui/SettingsDialog.java:500-545`
...
11. **Many bundled analysis scripts are hard-coded to one machine/user path and are not portable.**
    ...
    There are many scripts under `agent/scripts/` that hard-code `<developer-local experiment paths>` experiment and export paths. Those scripts will fail immediately outside that exact filesystem layout and are already inconsistent with the current repository owner's username.

## Low

12. **The TCP request-size guard enforces a character limit, but the error message claims a byte limit.**
    Refs: `src/main/java/imagejai/engine/TCPCommandServer.java:338-346`, `src/main/java/imagejai/engine/TCPCommandServer.java:590-603`
