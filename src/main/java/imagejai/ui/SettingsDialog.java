...
public class SettingsDialog extends JDialog {
    ...
    private void buildUI() {
        ...
        // Keep the profile form at its natural height. Other tabs are
        // scrollable, so they should not force this tab to show blank space.
        ...
        profileContent.add(profilePanel);
        profileContent.add(centerPanel);
    ...
    private void loadFromActiveConfig() {
        ...
        // Show the effective key: in-memory value, else the one relocated to the
        // <provider>.env secrets store (apiKey is no longer persisted to
        // config.json — verifier #3 / #4-1). Without this a migrated key shows
        // blank and a blind save would wipe it.
        String effKey = settings != null ? settings.resolveApiKey(editingConfig) : editingConfig.apiKey;
        if (effKey == null) effKey = "";
        geminiKeyField.setText("gemini".equals(p) ? effKey : "");
        ...
        openaiKeyField.setText("openai".equals(p) ? effKey : "");
        ...
        customKeyField.setText("custom".equals(p) ? effKey : "");
    ...
    /**
     * Persist every profile's non-empty key to its {@code <provider>.env} store.
     * Called ONLY from {@link #save()} on a confirmed dialog (verifier #5), so a
     * profile switch, a cancel, or a failed validation never writes or clears a
     * secret. Never clears — a blank field leaves the stored key untouched.
     */
    private void persistAllProfileKeys() {
        if (settings == null || settings.configs == null) {
            return;
        }
        for (imagejai.config.Settings.ModelConfig c : settings.configs) {
            if (c != null) {
                persistProfileKey(c.provider, c.apiKey);
            }
        }
    }

    /**
     * Persist a single legacy-profile key to the {@code <provider>.env} secrets
     * store so it survives a restart (apiKey is no longer written to config.json
     * — verifier #3 / #4-1). No-op for empty keys and ollama.
     */
    private void persistProfileKey(String provider, String key) {
        if (settings == null || provider == null
                || key == null || key.isEmpty() || "ollama".equals(provider)) {
            return;
        }
        try {
            imagejai.ui.installer.ProviderCredentials creds = settings.providerCredentials();
            if ("custom".equals(provider)) {
                java.util.Map<String, String> entries =
                        new java.util.LinkedHashMap<String, String>();
                entries.put(imagejai.config.Settings.CUSTOM_API_KEY_ENV, key);
                creds.saveEntries("custom", entries);
            } else if (imagejai.ui.installer.ProviderCredentials
                    .ENV_VAR_FOR_PROVIDER.containsKey(provider)) {
                creds.saveApiKey(provider, key);
            }
        } catch (Exception ex) {
            // Best-effort: the in-memory key still works for this session.
        }
    }
    ...
    private void save() {
        ...
        // Persist profile keys to the secrets store ONLY on a confirmed save
        // (verifier #5): apiKey is no longer written to config.json. Done here,
        // not in saveToActiveConfig(), so profile switches / cancels / failed
        // validation never write or clear secrets.
        persistAllProfileKeys();
     ...
     * icon (docs/multi_provider/06_tier_safety.md section 4.3).
     */
