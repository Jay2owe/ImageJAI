...
public final class TcpHotline {
    ...
    public JsonObject runScript(String language, String code, String source) throws IOException {
        JsonObject request = new JsonObject();
        request.addProperty("command", "run_script");
        request.addProperty("language", language == null || language.trim().isEmpty()
                ? "groovy"
                : language);
        request.addProperty("code", code);
        request.addProperty("source", source == null || source.trim().isEmpty()
