...
public class ProviderMenu extends JMenu {
    ...
    public interface StatusListener {
        ...
        default boolean onProviderStatusIconClicked(ProviderEntry entry,
                                                    ModelMenuItem.ProviderStatusIcon status) {
            return false;
        }
    }
    ...
    private void installHeaderStatusClick(final StatusListener statusListener) {
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                maybeHandleHeaderStatusClick(e);
            }

            private void maybeHandleHeaderStatusClick(MouseEvent e) {
                if (e == null || e.isConsumed()) {
                    return;
                }
                if (e.getX() < getWidth() - 28) {
                    return;
                }
                ModelMenuItem.ProviderStatusIcon icon = iconFor(providerEntry.status());
                if (statusListener.onProviderStatusIconClicked(providerEntry, icon)) {
                    e.consume();
                }
            }
        });
