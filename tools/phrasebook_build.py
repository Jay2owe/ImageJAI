...
def normalise(text: str) -> str:
    """Mirror uk.ac.ucl.imagej.ai.local.IntentMatcher.normalise."""
    text = text.lower()
    text = PUNCT.sub(" ", text)
...
def slugify(text: str) -> str:
    """Mirror uk.ac.ucl.imagej.ai.local.MenuIntentImporter.slugify."""
...
def java_load_check(json_path: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="phrasebook_java_load_") as temp:
        ...
        helper_src.write_text(
            "\n".join(
                [
                    "import uk.ac.ucl.imagej.ai.local.IntentLibrary;",
